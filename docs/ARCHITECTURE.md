...
| `reflect` | 0 | lib | Describes a POD struct's fields, so text serialisation, the command layer and an editor inspector are one mechanism rather than three. Today: `FieldType`, and the byte width and alignment of each type it names. Depends on `core` only — deliberately **not** `math`, so the vector widths are stated here rather than derived. The reflect gate cross-checks every stated width against the real `sizeof` |
