...
| `assets-gpu` | 2 | lib | CPU mesh → GPU buffers, and `GpuTextureStore`: CPU images → GPU textures keyed on the resolved content path, so a texture many materials share costs one upload |
