...
- `log(level, channel, message)` — channels: `General`, `Platform`, `Render`, `Assets`, `Audio`, `Physics`. Default logger is mutex-serialized stderr. `level_name` / `channel_name` are exported so every sink tags identically.
- `install_file_logger(dir)` (`core/log_file.hpp`) — installs a session sink that writes `<dir>/log.txt`, rotates any previous run to `log.prev.txt`, and **tees** to stderr, so console output is unchanged. **Flushes every line**, because the record worth keeping is the one written immediately before `std::abort()`. Returns false if the directory is not writable, leaving the stderr logger in place. Owns the sink for the life of the process: `main()`'s catch handlers log after `run_app()` returns. `default_log_directory(exe_dir)` is the one place the location is decided — Build #7 moves it to `%LOCALAPPDATA%`. `create_file_logger(dir)` is the same sink with caller-owned lifetime, which is what the gate uses. The apps install it after `create_platform()` and **not** under `--gates`, so gate runs cannot evict a real session's crash log from the two-file rotation.
...
- `ENGINE_ASSERT` / `ENGINE_ASSERT_MSG` — live in **both** Debug and Release (there is no `NDEBUG` guard), and abort. Reserve them for genuine programmer error — a violated precondition inside the engine. Capacity that depends on content (arena, frame ring, descriptor window, physics bodies, scene instances and materials) returns a failure instead, because aborting there is unrecoverable *and* untestable: no gate can exercise a path that calls `std::abort()`. "Inside the engine" is the operative half — `scene::add_instance` asserted on overflow until 31 Aug 2026, which made a *game's* spawn loop, not engine code, the thing that aborted; it returns `kInvalidInstance` and logs now, and `Scene capacity gate` asserts the degradation.
