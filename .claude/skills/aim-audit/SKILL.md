---
name: aim-audit
description: Full-depth audit of the engine — code (stability, architecture, capabilities, portability) and everything non-code (developer setup, AI tooling). Measures the tree, researches external ground truth, verifies every finding adversarially, grades six dimensions blind against the server's rubric, and submits one audit document to the management service, which validates it and renders every page. Also writes docs/analysis/PLAN.md, which /aim-fix applies. Expensive and deliberate — run it for the real picture, not during feature work.
...
service has no plan endpoints by design, and `/aim-fix` consumes this format as a
contract.
